  ...
  render() {
    return (
      <div id='parent' onClick={this.parentClick2}>
        ...
        {/* 若在原生事件上使用 stopPropagation 和 stopImmediatePropagation， 都能阻止冒泡到合成事件，只执行原生事件（原生事件不冒泡）。 */}
        {/* 若在合成事件上使用 stopPropagation 和 e.nativeEvent.stopImmediatePropagation，都无法阻止原生事件，先执行全部的原生事件（包括原生事件的冒泡），再执行合成事件(stopPropagation能阻止合成事件的冒泡，但stopImmediatePropagation无法阻止)，最后执行document上绑定的事件（stopPropagation无法阻止最外层document上绑定的事件（属于原生事件，且最后执行）,但stopImmediatePropagation可以阻止。 */}
