...
export default class Stop extends Component {
  ...
  childClick3 = (e)=> {
    console.log('child3');
    // e.stopPropagation();  //可以阻止合成事件的冒泡，但无法阻止最外层document上绑定的事件
  ...
  render() {
    return (
      <div id='parent' onClick={this.parentClick2}>
        ...
        {/* 在合成事件上使用 stopPropagation，原生事件执行并冒泡，包括document上绑定的事件也会执行，合成事件执行但不冒泡； */}
        {/* 在合成事件上使用 e.nativeEvent.stopImmediatePropagation，原生事件执行并冒泡，但document上的事件不执行，合成事件执行且冒泡； */}
